...
async function initDb() {
  ...
  // Drop legacy Phone.System (telecom.center) tables - no longer used after DIDWW removal
  try {
    await pool.query('DROP TABLE IF EXISTS phonesystem_termination_routes');
    await pool.query('DROP TABLE IF EXISTS phonesystem_termination_gateways');
    await pool.query('DROP TABLE IF EXISTS phonesystem_dids');
    await pool.query('DROP TABLE IF EXISTS phonesystem_incoming_trunks');
    await pool.query('DROP TABLE IF EXISTS phonesystem_customers');
    if (DEBUG) console.log('[schema] Dropped legacy Phone.System tables');
  } catch (e) {
    if (DEBUG) console.warn('[schema] Failed to drop Phone.System tables:', e.message || e);
  }
  ...
  try {
    ...
    for (const c of campaignRows) {
      ...
      if (available && DIALER_GLOBAL_MAX_DIALOUT > 0) {
        ...
        if (!globalAvailable) {
          if (DEBUG) {
            console.log('[dialer.worker] Global dial-out cap reached; skipping campaign', {
              campaignId,
              userId,
              inProgress: globalInProgress,
              max: DIALER_GLOBAL_MAX_DIALOUT
            });
          }
          continue;
        }
        available = Math.max(0, Math.min(available, globalAvailable));
      }
